...
## Step 3: Add OmniAuth gems and dotenv
...
Google access tokens expire after about 1 hour. In this tutorial, when a token expires, we simply sign the user out and ask them to re-authenticate. This keeps things simple for learning purposes.

In a production app, you would use the refresh token to automatically get a new access token without requiring user interaction. Google only sends the refresh token on the *first* authorization (or when you force re-consent with `prompt: "consent"`), so it's important to store it. We store it in this tutorial, but don't implement automatic refresh - that's left as an exercise for the reader.
...
```
bin/rails generate migration AddOmniAuthColumnsToUsers provider:string uid:string google_access_token:string google_refresh_token:string
```

This creates a migration that adds columns:
...
- `google_refresh_token` - stores the token used to refresh the access token when it expires
...
For a production application, you should strictly avoid storing access tokens in plain text. Rails 7+ makes this easy with `encrypts`:
...
- The `from_omniauth` class method:
  ...
  - **Always updates** the `google_access_token` on every sign-in
  - Updates `google_refresh_token` if one is provided (usually only on the first sign-in)
...
<% if @events.any? %>
  <ul>
    <% @events.each do |event| %>
      <li>
        <strong><%= link_to (event.summary || "(No title)"), event.html_link, target: "_blank" %></strong>
        <br>
...
- Loops through `@events` and displays each event's title (linked to Google Calendar) and start time
- Handles events with no title by showing "(No title)"
- Handles both timed events and all-day events

## Step 13: Add a "Sign in with Google" button

Devise's default sign-in page shows email/password fields, but we want users to sign in with Google. Let's generate Devise's views so we can customize them:

```
bin/rails generate devise:views
```

This creates view templates in `app/views/devise/`. Open `app/views/devise/sessions/new.html.erb` and add a Google sign-in button. Add this after the form:
...
**Important**: We use `button_to` instead of `link_to` because OmniAuth requires a POST request for security (CSRF protection). The `data: { turbo: false }` prevents Turbo from intercepting the request.
...
### "Not Found" or "No route matches [GET]" when signing in

The `omniauth-rails_csrf_protection` gem requires OAuth requests to be `POST` requests for security.
- If you are using Devise's default views, this should work automatically.
- If you are creating your own link, use `button_to` (which creates a form) instead of `link_to`:
  ```erb
  <%= button_to "Sign in with Google", user_google_oauth2_omniauth_authorize_path, data: { turbo: false } %>
  ```
